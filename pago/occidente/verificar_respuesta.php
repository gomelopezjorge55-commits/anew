...
// Evitar acciones repetidas con offset
...
if (isset($data['result']) && is_array($data['result'])) {
    foreach ($data['result'] as $update) {
        ...
        // Obtener acción desde callback_data
        list($actionType, ) = explode(':', $callback['data'], 2);
        $action = $actionType;

        // Marcar este update como procesado
        $_SESSION['last_update_id'] = (int)$update['update_id'];

        // Texto original del mensaje de Telegram
        ...
        switch ($actionType) {
            ...
            case 'confirm_finalizar':
                ...
                break;
            default:
                // Por si en el futuro agregas algo tipo "sms", "llamar_cliente", etc.
                $accionTexto = ucfirst(str_replace('_', ' ', $actionType));
                break;
        }
        ...
        // Editar el mensaje: texto + eliminar inline keyboard
        $payload = [
            'chat_id'    => $chatId,
